...
func TestCodegen(t *testing.T) {
	...
	expectedFiles := []string{
		...
		"rstf/generated/routes/live-chat._id.ts",
	...
	assert.Contains(t, routesModStr, `"live-chat._id": {`)
	assert.Contains(t, routesModStr, `GetMessages: defineQuery<{ id: string }, RoutesLiveChatId.GetMessagesResult>("live-chat._id", "GetMessages")`)
	assert.Contains(t, routesModStr, `SendMessage: defineMutation<{ id: string }, RoutesLiveChatId.SendMessageInput, void>("live-chat._id", "SendMessage")`)
	assert.Contains(t, routesModStr, `"users._id": {`)
	...
	assert.Contains(t, goRoutesStr, `func (UsersDotIdRoute) Name() string { return "users._id" }`)
	...
	assert.Contains(t, result.Entries, "routes/live-chat._id")

...
function Example() {
  ...
  useMutation(routes["live-chat.$id"].GetMessages, { id: "room-1" });
  // @ts-expect-error mutations cannot be passed to useQuery
  useQuery(routes["live-chat.$id"].SendMessage, { id: "room-1" });
  // @ts-expect-error actions require useAction
  useMutation(routes["live-chat.$id"].EchoAction, { id: "room-1" });

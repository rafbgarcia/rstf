...
// and runtime modules.
//
// It walks a directory tree looking for .go files that export an SSR function
// returning a struct. It generates TypeScript interfaces and runtime modules
// that export a serverData() function for use in React components.
package codegen
...
func GenerateRuntimeModule(rf RouteFile) string {
	...
	fn := rf.Funcs[0]
	var returnStruct *StructDef
	for i := range rf.Structs {
		if rf.Structs[i].Name == fn.ReturnType {
			returnStruct = &rf.Structs[i]
			break
		}
	}
	if returnStruct == nil {
	...
	// serverData() function
	fmt.Fprintf(&b, "export function serverData(): %s.%s {\n", ns, returnStruct.Name)
	b.WriteString("  return {\n")
	for _, f := range returnStruct.Fields {
		fmt.Fprintf(&b, "    %s: _data.%s ?? %s,\n", f.JSONName, f.JSONName, tsDefaultValue(f.Type))
	}
...
// tsDefaultValue returns the TypeScript default value for a given TS type.
func tsDefaultValue(tsType string) string {
	switch {

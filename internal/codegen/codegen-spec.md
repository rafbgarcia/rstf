...
.rstf/
  ...
  static/
    ...
    dashboard/bundle.js        # Hydration bundle (generated by bun build)
```
...
The `tsconfig.json` `paths` mapping handles this for TypeScript. Bun also respects `tsconfig.json` paths at runtime, so the same mapping works for SSR and client-side bundling.
...
9. Resolve framework dependencies via `go get` — `server_gen.go` lives in `.rstf/` (dot-prefixed, invisible to `go mod tidy`), so its transitive deps (e.g. chi via `rstf/router`) must be added to `go.sum` explicitly
